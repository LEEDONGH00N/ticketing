...
import study.kiwi.ticketing.global.auth.token.JwtProvider;
import study.kiwi.ticketing.global.auth.token.vo.RefreshTokenVO;
import study.kiwi.ticketing.global.auth.token.vo.TokenResponse;
...
public class MemberServiceImpl implements MemberService {
    ...
    @Transactional
    public String signupAfterNaverLogin(String oauthId) {

        Member member = memberRepository.findByOauthIdAndProviderType(oauthId, OAuthProviderType.NAVER)
                .orElseThrow(() -> new BaseException(MEMBER_NOT_FOUND));

        log.info("member role : {}", member.getRole());
        member.memberSignupDone();
        return "회원가입 완료";
    }
